...
def get_business_trends(location, radius=3000, user_id=None):
    """
    Analyze business types in the given area using Google Places API, filtering out generic types.
    """
    endpoint = "https://maps.googleapis.com/maps/api/place/nearbysearch/json"
    all_types = []

    # Define business-relevant types (manually curated)
    ...
    # Store to DB if user_id is available
    if user_id:
        try:
            db = get_db()
            cursor = db.cursor()
...
def download_report(report_id):
    ...
    try:
        ...
        db = get_db()
        cursor = db.cursor()
        cursor.execute(f"SELECT * FROM generated_reports WHERE id = {report_id}")
